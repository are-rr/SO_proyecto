...
void insertar(struct Nodo **cabeza, int pid,int gid,FILE *archivo,const char *nombre,char status, int pc) {
    ...
    strncpy(nuevo->nombrePro, nombre, sizeof(nuevo->nombrePro) - 1); //copea el texto de "nombre" que viene de "archivo"(en nuevo->nuevoPro)
...
int validarPC(FILE *copiaArchivo,int pc_buscar){
    char buffer[200];//arreglo temporar para guardar cada linea que se lee con fgets

    for(int i=0;i<pc_buscar;i++){
        if(fgets(buffer,sizeof(buffer), copiaArchivo)==NULL){//lee una linea, guarda esa linea en buffer y se mueve el cursos a la sig linea
            return 0;
        }  

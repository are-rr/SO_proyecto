...
void imprimirProceso(struct Nodo *p,int y_ncurse){
    ...
    if(p->Status == 'E'){
        mvprintw(y_ncurse,0,"%-5d %-5d %-18s %-18s %-10d %-18s %10d %10d %10d %10d %-8d %-8d",
            p->PID,p->GID,p->nombrePro,statusTexto(p->Status),p->PC,p->IR,p->EAX,p->EBX,p->ECX,p->EDX,p->CPU,p->GCPU);
    }
    else if(p->Status == 'L'){
        mvprintw(y_ncurse,0,"%-5d %-5d %-18s %-18s %-10d %-18s %10d %10d %10d %10d %-8d %-8d",
            p->PID,p->GID,p->nombrePro,statusTexto(p->Status),p->PC,p->IR,p->EAX,p->EBX,p->ECX,p->EDX,p->CPU,p->GCPU);
